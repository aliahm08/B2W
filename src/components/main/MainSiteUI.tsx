...
export function MainButton({ to, children, variant = 'primary', surface = 'light' }: { to: string; children: ReactNode; variant?: 'primary' | 'secondary' | 'text'; surface?: 'light' | 'dark' }) {
  const style = surface === 'dark'
    ? variant === 'primary' ? 'bg-white text-[#18201b] hover:bg-[#e7eef1]' : variant === 'secondary' ? 'border border-white/25 bg-white/8 text-white hover:border-white/50 hover:bg-white/12' : 'text-[#a9c7d4] underline underline-offset-4'
    : variant === 'primary' ? 'bg-[#18201b] text-white hover:bg-[#315f79]' : variant === 'secondary' ? 'border border-black/15 bg-white text-[#18201b] hover:border-black/35' : 'text-[#315f79] underline underline-offset-4';
  const className = `inline-flex min-h-12 items-center justify-center gap-2 rounded-full px-5 text-sm font-bold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315f79] focus-visible:ring-offset-2 ${surface === 'dark' ? 'focus-visible:ring-offset-[#111614]' : ''} ${style}`;
...
export function MainHero({ eyebrow, title, description, primary = { label: 'Try JasonAI', to: '/get-started' }, secondary = { label: 'See How It Works', to: '/jasonai/how-it-works' }, aside, dark = false, grain = false }: { eyebrow: string; title: string; description: string; primary?: { label: string; to: string } | null; secondary?: { label: string; to: string } | null; aside?: ReactNode; dark?: boolean; grain?: boolean }) {
  const noise = "url(\"data:image/svg+xml,%3Csvg viewBox='0 0 180 180' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='.82' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='.82'/%3E%3C/svg%3E\")";
  return <section className={`relative isolate overflow-hidden ${dark ? 'bg-[#111614] text-white' : ''}`}>{grain ? <div aria-hidden="true" className="pointer-events-none absolute inset-0 opacity-[.075] mix-blend-soft-light" style={{ backgroundImage: noise, backgroundSize: '180px 180px' }} /> : null}<div className={`${mainPageWidth} relative z-10 pb-20 pt-36 sm:pb-28 sm:pt-44`}><div className={`grid gap-10 ${aside ? 'lg:grid-cols-[1.02fr_.98fr] lg:items-center' : ''}`}><div><p className={`text-xs font-bold uppercase tracking-[.16em] ${dark ? 'text-[#9cc0d1]' : 'text-[#315f79]'}`}>{eyebrow}</p><h1 className="mt-6 max-w-[13ch] text-[clamp(3.25rem,8vw,7.5rem)] font-semibold leading-[.9] tracking-[-.065em]">{title}</h1><p className={`mt-7 max-w-2xl text-lg leading-8 sm:text-xl ${dark ? 'text-white/65' : 'text-[#18201b]/65'}`}>{description}</p>{primary || secondary ? <div className="mt-8 flex flex-wrap gap-3">{primary ? <MainButton to={primary.to} surface={dark ? 'dark' : 'light'}>{primary.label}</MainButton> : null}{secondary ? <MainButton to={secondary.to} variant="secondary" surface={dark ? 'dark' : 'light'}>{secondary.label}</MainButton> : null}</div> : null}</div>{aside ? <div>{aside}</div> : null}</div></div></section>;
}
